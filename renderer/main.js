...
FollowFromUpCamera = function () {
	...
	this.eyePos = glMatrix.vec3.fromValues(0, 25, 0);
...
ChaseCamera = function () {
	...
	//posizione del punto di vista attuale
	this.eyePos = glMatrix.vec3.fromValues(0, 4, 8);
	//posizione del target attuale
	this.targetPos = glMatrix.vec3.fromValues(0, 1, 0);
...
Renderer.initializeObjects = function (gl) {
	...
	this.cube2 = new Cube2inv(1);
	ComputeNormals(this.cube2);
	this.createObjectBuffers(gl, this.cube2);
	...
	this.textureCubeMap = createCubeMap(nTexture++, gl,//15
		"../common/textures/cubemap/posx2.jpg",
		"../common/textures/cubemap/negx2.jpg",
		"../common/textures/cubemap/posy2.jpg",
		"../common/textures/cubemap/negy2.jpg",
		"../common/textures/cubemap/posz2.jpg",
		"../common/textures/cubemap/negz2.jpg");
...
Renderer.drawCubemap = function(){
	...
	gl.uniformMatrix4fv(this.shaderCubemap.uViewMatrixLocation, false, glMatrix.mat4.lookAt(glMatrix.mat4.create(), [0.0,0.0,0.0], Renderer.cameras[Renderer.currentCamera].vista(), Renderer.cameras[Renderer.currentCamera].up()));
	...
	gl.uniform1i(this.shaderCubemap.uSamplerCMLocation, 15);//cubemap text

	gl.depthMask(false);
	
	gl.bindBuffer(gl.ARRAY_BUFFER, this.cube2.vertexBuffer);
	...
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.cube2.indexBufferTriangles);
	gl.drawElements(gl.TRIANGLES, this.cube2.triangleIndices.length, gl.UNSIGNED_SHORT, 0);
...
Renderer.drawScene = function () {
	...
	/////////////////////////iniziamo a disegnare la scena
	//modifica4
	//matrici vista e proiezione
	Renderer.cameras[Renderer.currentCamera].update(this.car.frame);
	var viM = Renderer.cameras[Renderer.currentCamera].matrix();
	var prM = glMatrix.mat4.perspective(glMatrix.mat4.create(), Math.PI/ 4, canvasW / canvasH, 1, 250);


	////////////////////////////////////////cubemap
	this.drawCubemap(viM, prM);

	///////////////////////////////////disegnamo veramente la scena vera
	gl.useProgram(this.uniformShader);

	//matrice proiezione
	gl.uniformMatrix4fv(this.uniformShader.uProjectionMatrixLocation, false, prM);

	//matrice di vista
	gl.uniformMatrix4fv(this.uniformShader.uViewMatrixLocation, false, viM);
	...
	for(let n = 0; n<12; n++){
		//modifica4
		let dummy1 = glMatrix.vec4.transformMat4(glMatrix.vec4.create(), this.posArr[n], viM);
		let dummy2 = glMatrix.vec4.transformMat4(glMatrix.vec4.create(), this.dirArr[n], viM);
